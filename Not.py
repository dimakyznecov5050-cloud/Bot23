...
# ---------- ТОКЕН ----------
# ВСТАВЬ сюда свой токен (который ты прислал) и НЕ публикуй его нигде
...
def process_player_id(message, uc_amount, final_price, original_price, discount, promo_code):
    ...
    # --- ПРОВЕРКА: только цифры, минимум 5 символов, и ID должен начинаться с '5'
    if (not player_id.isdigit()) or (len(player_id) < 5) or (not player_id.startswith('5')):
        bot.send_message(
            message.chat.id,
            "❌ <b>ОШИБКА!</b>\n\n"
            "Введите корректный ID (только цифры, минимум 5 цифр, и ID должен начинаться с <b>5</b>).\n"

...
API_TOKEN = "5d1076f7-e6a6-48a2-843a-8fbd81640b86"

def send_uc_to_supplier(product_id, player_id):
    url = "https://hutao.syntex.az/api/v1/pubg/id/order"
...
def handle_callback(call):
    ...
    if data.startswith('admin_done_'):
        ...
        conn = get_conn()
        c = conn.cursor()
        c.execute('SELECT player_id, quantity_text FROM orders WHERE order_number = ?', (order_number,))
        ...
        quantity_text = order_data["quantity_text"]
        ...
        product_id = uc_map.get(quantity_text)

        if not product_id:
            bot.send_message(ADMIN_ID, f"❌ Не найден product_id для {quantity_text}")
            return

        response = send_uc_to_supplier(product_id, player_id)

        if "error" in response:
            bot.send_message(ADMIN_ID, f"❌ API ошибка: {response['error']}")
            return

        order = finalize_order_success(order_number)

        bot.send_message(order['user_id'], f'✅ UC отправлены!')
        bot.edit_message_text(f'✅ ЗАКАЗ №{order_number} ВЫПОЛНЕН', call.message.chat.id, call.message.message_id)
        return
        order_number = int(data.split('_')[2])
        order = finalize_order_success(order_number)
        if not order:

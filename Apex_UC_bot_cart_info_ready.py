...
def build_uc_cart_text(cart, discount=0, promo_code=None):
    ...
    if cart:
        ...
        lines.append('Корзина пока пуста. Нажимайте на кнопки ниже, чтобы добавить паки.')
    lines.extend([
        '',
        'Нажмите на нужный пак, чтобы добавить его в корзину. Удалить можно кнопкой «➖».',
    ])
    return '\n'.join(lines)


def build_uc_cart_markup(cart):
    cart, total_uc, total_price, total_packs = calc_uc_cart(cart)
    ...
    for uc_amount in sorted(UC_PRICES):
        ...
        markup.add(build_inline_button(f'➕ {label}', f'ucadd_{uc_amount}', '5253558615842785529'))
        markup.add(build_inline_button(f'➖ Убрать {uc_amount} UC', f'ucrem_{uc_amount}', '5255732655273570754'))
    if cart:
...
def handle_callback(call):
    ...
    if data == 'menu_uc':
        ...
        set_state(user_id, uc_cart=cart, awaiting=None)
        return
    ...
    if data == 'uc_clear':
        set_state(user_id, uc_cart={}, awaiting=None)

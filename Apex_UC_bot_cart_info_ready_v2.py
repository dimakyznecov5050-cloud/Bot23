...
def build_uc_cart_text(cart, discount=0, promo_code=None):
    ...
    if cart:
        ...
        if discount:
            ...
            lines.extend([
                '',
                f'🎟 Промокод: <b>{escape_html(promo_code or "активен")}</b> (-{discount}%)',
                f'💳 К оплате со скидкой: <b>{fmt_price(discounted)} ₽</b>'
            ])
    else:
        lines.append('Корзина пока пуста. Нажимайте на нужные паки ниже, чтобы добавить их.')
    lines.extend([
        '',
        'Нажмите на нужный пак. После выбора его строка разделится на две кнопки: добавить и удалить 1.',
    ])
    return '\n'.join(lines)
...
def build_uc_cart_markup(cart, active_uc=None):
    ...
    for uc_amount in sorted(UC_PRICES):
        ...
        count = cart.get(uc_amount, 0)
        label = f'{uc_amount} UC — {fmt_price(price)} ₽'
        if count:
            label += f' | в корзине: {count}'
        if active_uc == uc_amount and count > 0:
            markup.row(
                build_inline_button(f'{uc_amount} UC +1', f'ucadd_{uc_amount}', '5253558615842785529'),
                build_inline_button('Удалить 1', f'ucrem_{uc_amount}', '5255732655273570754')
            )
        else:
            markup.add(build_inline_button(label, f'ucadd_{uc_amount}', '5253558615842785529'))
    if cart:
        markup.add(build_inline_button('✅ Перейти к вводу PUBG ID', 'uc_checkout', '5253558615842785529'))
...
def handle_callback(call):
    ...
    if data == 'menu_uc':
        state = get_state(user_id)
        cart = state.get('uc_cart', {})
        active_uc = state.get('active_uc')
        edit_menu(user_id, build_uc_cart_text(cart), build_uc_cart_markup(cart, active_uc=active_uc))
        set_state(user_id, uc_cart=cart, awaiting=None, active_uc=active_uc)
        return

    if data.startswith('ucadd_'):
        uc_amount = int(data.split('_')[1])
        state = get_state(user_id)
        cart = dict(state.get('uc_cart', {}))
        cart[uc_amount] = int(cart.get(uc_amount, 0)) + 1
        set_state(user_id, uc_cart=cart, awaiting=None, active_uc=uc_amount)
        edit_menu(user_id, build_uc_cart_text(cart), build_uc_cart_markup(cart, active_uc=uc_amount))
        return
    ...
    if data.startswith('ucrem_'):
        ...
        if current_count <= 1:
            cart.pop(uc_amount, None)
        else:
            cart[uc_amount] = current_count - 1
        active_uc = uc_amount if cart.get(uc_amount, 0) > 0 else None
        set_state(user_id, uc_cart=cart, awaiting=None, active_uc=active_uc)
        edit_menu(user_id, build_uc_cart_text(cart), build_uc_cart_markup(cart, active_uc=active_uc))
        return
    ...
    if data == 'uc_clear':
        ...
        edit_menu(user_id, build_uc_cart_text({}), build_uc_cart_markup({}, active_uc=None))
        return
    ...
    if data == 'uc_checkout':
        ...
        if promo:
            ...
            final_price = int(total_price * (100 - discount) / 100)
    ...
    if data == 'menu_info':
        conn = get_conn()
        c = conn.cursor()
        c.execute('SELECT username, first_name, total_uc, total_orders FROM users WHERE user_id = ?', (user_id,))
        row = c.fetchone()
        ...
        text = (
            '<tg-emoji emoji-id="5447410659077661506">ℹ️</tg-emoji> <b>ПРОФИЛЬ ПОКУПАТЕЛЯ</b>\n\n'
...
def handle_uc_player_id(message, state):
    ...
    clear_state(message.from_user.id, keep_menu=True)
    edit_menu(message.from_user.id,
              build_payment_text(order_number, 'UC', draft['quantity_text'], draft['price'], player_id, 'player_id', uc_amount=draft['uc_amount']),

...
POPULARITY_ITEMS = {
    ...
    'pop_home': {
        ...
        'description': '✨<b>Популярность для дома</b>✨\n\n🕒Среднее время доставки 1-15 минут',
    ...
    'pop_last': {
        ...
        'description': '✨<b>Популярность для последних минут раунда</b>✨\n\nОформляйте заказ заранее.',
...
def send_or_update_main_menu(chat_id, user_id):
    ...
    if mid and mchat == chat_id:
        try:
            bot.edit_message_caption(text, chat_id, mid, reply_markup=markup)
            clear_state(user_id, keep_menu=True)
            set_state(user_id, menu_chat_id=chat_id, menu_message_id=mid)
            return
        except Exception:
            pass
...
def edit_menu(user_id, text, reply_markup):
    ...
    if not cid or not mid:
        return False
    try:
        bot.edit_message_caption(text, cid, mid, reply_markup=reply_markup)
        return True
    except Exception:
        try:

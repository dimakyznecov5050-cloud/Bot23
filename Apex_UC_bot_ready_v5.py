...
def menu_keyboard():
    ...
    # В тексте inline-кнопок Telegram Bot API не поддерживает custom emoji entities,
    # поэтому подписи кнопок оставлены обычным текстом без изменения логики.
    ...
    markup.add(types.InlineKeyboardButton('Пополнить Telegram звезды', callback_data='menu_tgstars'))
    markup.add(
        types.InlineKeyboardButton('Поддержка', callback_data='menu_support'),
        types.InlineKeyboardButton('Отзывы', callback_data='menu_reviews')
    )
    return markup
...
def render_main_menu_text(first_name='Игрок'):
    return (
        '<tg-emoji emoji-id="5404617696589390973">👋</tg-emoji> <b>ДОБРО ПОЖАЛОВАТЬ В APEX UC SHOP!</b>

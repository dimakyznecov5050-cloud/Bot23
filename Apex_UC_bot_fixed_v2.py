...
def get_main_menu_image_path():
    candidates = [
        MAIN_MENU_IMAGE,
        os.path.join(os.getcwd(), MAIN_MENU_IMAGE),
        os.path.join(os.path.dirname(__file__), MAIN_MENU_IMAGE),
        os.path.join('/mnt/data', MAIN_MENU_IMAGE),
    ]
...
def clear_state(user_id, keep_menu=True):
    with state_lock:
        ...
        menu_photo_message_id = current.get('menu_photo_message_id') if keep_menu else None
        user_states[user_id] = {}
        if keep_menu and menu_chat_id and menu_message_id:
            ...
            if menu_photo_message_id:
                user_states[user_id]['menu_photo_message_id'] = menu_photo_message_id
...
def send_or_update_main_menu(chat_id, user_id, first_name='Игрок'):
    ...
    photo_message_id = state.get('menu_photo_message_id')
    image_path = get_main_menu_image_path()

    if photo_message_id and menu_chat_id == chat_id:
        try:
    ...
    if image_path:
        try:
            with open(image_path, 'rb') as photo:
                sent_photo = bot.send_photo(chat_id, photo)
            photo_id = sent_photo.message_id
        except Exception:
            photo_id = None
    ...
    if menu_message_id and menu_chat_id == chat_id:
        try:
            ...
            if photo_id:
                set_state(user_id, menu_chat_id=chat_id, menu_message_id=menu_message_id, menu_photo_message_id=photo_id)
            else:
    ...
    if photo_id:
        set_state(user_id, menu_chat_id=chat_id, menu_message_id=sent.message_id, menu_photo_message_id=photo_id)
    else:
...
def edit_menu(user_id, text, reply_markup):
    ...
    if photo_message_id:
        try:
            ...
            pass
        set_state(user_id, menu_photo_message_id=None)
    try:

...
DB_PATH = os.path.join(BASE_DIR, 'uc_bot.db')
MAIN_MENU_IMAGE = os.path.join(BASE_DIR, 'IMG_2822.jpeg')
FALLBACK_MENU_IMAGE = os.path.join(BASE_DIR, 'IMG_2811.png')
TG_STARS_PRICE = 1.5
...
def load_prices_from_db():
    global TG_STARS_PRICE
    for uc in list(UC_PRICES.keys()):
        saved = get_setting(f'price_uc_{uc}')
        if saved is not None:
            UC_PRICES[uc] = int(float(saved))
    for key, config in POPULARITY_ITEMS.items():
        updated = []
        for idx, (label, price) in enumerate(config['prices']):
            saved = get_setting(f'price_pop_{key}_{idx}')
            updated.append((label, int(float(saved)) if saved is not None else price))
        config['prices'] = updated
    updated_subs = []
    for idx, (label, price) in enumerate(SUBSCRIPTION_ITEMS):
        saved = get_setting(f'price_sub_{idx}')
        updated_subs.append((label, int(float(saved)) if saved is not None else price))
    SUBSCRIPTION_ITEMS[:] = updated_subs
    saved_star = get_setting('price_tg_star_unit')
    if saved_star is not None:
        TG_STARS_PRICE = float(saved_star)


def save_uc_price(uc_amount, price):
    UC_PRICES[uc_amount] = price
    set_setting_value(f'price_uc_{uc_amount}', price)


def save_popularity_price(category_key, idx, price):
    label, _ = POPULARITY_ITEMS[category_key]['prices'][idx]
    POPULARITY_ITEMS[category_key]['prices'][idx] = (label, price)
    set_setting_value(f'price_pop_{category_key}_{idx}', price)


def save_subscription_price(idx, price):
    label, _ = SUBSCRIPTION_ITEMS[idx]
    SUBSCRIPTION_ITEMS[idx] = (label, price)
    set_setting_value(f'price_sub_{idx}', price)


def save_tg_star_price(price):
    global TG_STARS_PRICE
    TG_STARS_PRICE = price
    set_setting_value('price_tg_star_unit', price)
...
def get_menu_image_path():
    for path in (MAIN_MENU_IMAGE, FALLBACK_MENU_IMAGE):
...
def send_or_update_main_menu(chat_id, user_id, first_name='Игрок'):
    ...
    menu_is_photo = state.get('menu_is_photo', False)
    image_path = get_menu_image_path()

    if menu_message_id and menu_chat_id == chat_id:
        try:
            if image_path:
                if menu_is_photo:
                    bot.edit_message_caption(text, chat_id, menu_message_id, reply_markup=markup)
                else:
                    sent = bot.send_photo(chat_id, open(image_path, 'rb'), caption=text, reply_markup=markup)
                    safe_delete_message(chat_id, menu_message_id)
                    clear_state(user_id, keep_menu=False)
                    set_state(user_id, menu_chat_id=chat_id, menu_message_id=sent.message_id, menu_is_photo=True)
                    return
            else:
                bot.edit_message_text(text, chat_id, menu_message_id, reply_markup=markup)
            clear_state(user_id, keep_menu=True)
            set_state(user_id, menu_chat_id=chat_id, menu_message_id=menu_message_id, menu_is_photo=bool(image_path))
            return
        except Exception:
            pass
    ...
    if image_path:
        ...
        clear_state(user_id, keep_menu=False)
        set_state(user_id, menu_chat_id=chat_id, menu_message_id=sent.message_id, menu_is_photo=False)
...
def mask_name(name):
    name = (name or 'Игрок').strip()
    if len(name) <= 2:
        return name[0] + '*' if len(name) == 2 else '*'
    visible = max(2, len(name) // 2)
    return escape_html(name[:visible] + '*' * (len(name) - visible))
...
def admin_command(message):
    if not is_admin(message.from_user.id):
        ...
        return
    sent = bot.send_message(message.chat.id, ADMIN_MENU_TEXT, reply_markup=admin_keyboard())
    set_state(message.from_user.id, admin_chat_id=sent.chat.id, admin_message_id=sent.message_id)
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('pricecat_') or call.data.startswith('pricepick_'))
def admin_price_callbacks(call):
    ...
    if data == 'pricecat_uc':
        ...
        for uc, price in sorted(UC_PRICES.items()):
            markup.add(types.InlineKeyboardButton(f'{uc} UC — {fmt_price(price)} ₽', callback_data=f'pricepick_uc_{uc}'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='admin_prices'))
        markup.add(types.InlineKeyboardButton('🚪 Выйти', callback_data='admin_exit'))
        bot.edit_message_text('💸 <b>Изменение цен — UC</b>\n\nВыберите пакет:', call.message.chat.id, call.message.message_id, reply_markup=markup)
        return

    if data == 'pricecat_subs':
        markup = types.InlineKeyboardMarkup(row_width=1)
        for idx, (label, price) in enumerate(SUBSCRIPTION_ITEMS):
            markup.add(types.InlineKeyboardButton(f'{label} — {fmt_price(price)} ₽', callback_data=f'pricepick_sub_{idx}'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='admin_prices'))
        markup.add(types.InlineKeyboardButton('🚪 Выйти', callback_data='admin_exit'))
        bot.edit_message_text('💸 <b>Изменение цен — Подписки</b>\n\nВыберите пакет:', call.message.chat.id, call.message.message_id, reply_markup=markup)
        return

    if data == 'pricecat_pop':
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton('✨ Популярность', callback_data='pricepick_popcat_pop_regular'))
        markup.add(types.InlineKeyboardButton('🏠 Популярность для дома', callback_data='pricepick_popcat_pop_home'))
        markup.add(types.InlineKeyboardButton('⏱ Последние минуты', callback_data='pricepick_popcat_pop_last'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='admin_prices'))
        markup.add(types.InlineKeyboardButton('🚪 Выйти', callback_data='admin_exit'))
        bot.edit_message_text('💸 <b>Изменение цен — Популярность</b>\n\nВыберите раздел:', call.message.chat.id, call.message.message_id, reply_markup=markup)
        return

    if data.startswith('pricepick_popcat_'):
        key = data.split('pricepick_popcat_', 1)[1]
        config = POPULARITY_ITEMS[key]
        markup = types.InlineKeyboardMarkup(row_width=1)
        for idx, (label, price) in enumerate(config['prices']):
            markup.add(types.InlineKeyboardButton(f'{label} — {fmt_price(price)} ₽', callback_data=f'pricepick_pop_{key}_{idx}'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='pricecat_pop'))
        markup.add(types.InlineKeyboardButton('🚪 Выйти', callback_data='admin_exit'))
        bot.edit_message_text(f'💸 <b>{escape_html(config["title"])} — изменение цен</b>\n\nВыберите пакет:', call.message.chat.id, call.message.message_id, reply_markup=markup)
        return

    if data == 'pricecat_stars':
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton(f'1 звезда — {fmt_price(TG_STARS_PRICE)} ₽', callback_data='pricepick_star_unit'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='admin_prices'))
        markup.add(types.InlineKeyboardButton('🚪 Выйти', callback_data='admin_exit'))
        bot.edit_message_text('💸 <b>Изменение цен — Telegram Stars</b>\n\nВыберите, что менять:', call.message.chat.id, call.message.message_id, reply_markup=markup)
        return

    if data == 'pricepick_star_unit':
        msg = bot.send_message(call.message.chat.id, 'Введите новую цену за <b>1 звезду</b>:')
        bot.register_next_step_handler(msg, lambda m: process_new_price_value(m, ('star_unit', None)))
        return

    if data.startswith('pricepick_uc_'):
        uc_amount = int(data.split('_')[2])
        msg = bot.send_message(call.message.chat.id, f'Введите новую цену для <b>{uc_amount} UC</b>:')
        bot.register_next_step_handler(msg, lambda m, uc_amount=uc_amount: process_new_price_value(m, ('uc', uc_amount)))
        return

    if data.startswith('pricepick_sub_'):
        idx = int(data.split('_')[2])
        label, _ = SUBSCRIPTION_ITEMS[idx]
        msg = bot.send_message(call.message.chat.id, f'Введите новую цену для <b>{escape_html(label)}</b>:')
        bot.register_next_step_handler(msg, lambda m, idx=idx: process_new_price_value(m, ('sub', idx)))
        return

    if data.startswith('pricepick_pop_'):
        payload = data.replace('pricepick_pop_', '', 1)
        key, idx = payload.rsplit('_', 1)
        idx = int(idx)
        label, _ = POPULARITY_ITEMS[key]['prices'][idx]
        msg = bot.send_message(call.message.chat.id, f'Введите новую цену для <b>{escape_html(label)}</b>:')
        bot.register_next_step_handler(msg, lambda m, key=key, idx=idx: process_new_price_value(m, ('pop', key, idx)))
        return


def process_new_price_value(message, target):
    if not is_admin(message.from_user.id):
        return
    try:
        ...
        bot.send_message(message.chat.id, '❌ Введите корректную цену больше 0.')
        return

    target_type = target[0]
    if target_type == 'uc':
        save_uc_price(target[1], int(price))
        bot.send_message(message.chat.id, f'✅ Цена для {target[1]} UC изменена на {fmt_price(int(price))} ₽.')
    elif target_type == 'sub':
        save_subscription_price(target[1], int(price))
        bot.send_message(message.chat.id, f'✅ Цена подписки изменена на {fmt_price(int(price))} ₽.')
    elif target_type == 'pop':
        save_popularity_price(target[1], target[2], int(price))
        bot.send_message(message.chat.id, f'✅ Цена пакета популярности изменена на {fmt_price(int(price))} ₽.')
    elif target_type == 'star_unit':
        save_tg_star_price(price)
        bot.send_message(message.chat.id, f'✅ Цена за 1 звезду изменена на {fmt_price(price)} ₽.')
...
def handle_callback(call):
    ...
    if data.startswith('popsel_'):
        payload = data.replace('popsel_', '', 1)
        key, idx = payload.rsplit('_', 1)
        config = POPULARITY_ITEMS[key]
    ...
    if data == 'menu_info':
        text = (
            'ℹ️ <b>Информация</b>\n\n'
            '🕒 Доставка обычно занимает 5-15 минут.\n'
            '✅ После оплаты нажмите кнопку «Я оплатил».\n'
            '📞 Если возникли вопросы — напишите в поддержку.\n'
            '💳 Оплата доступна по реквизитам, которые бот покажет после выбора товара.'
        )
    ...
    if data == 'menu_leaders':
        ...
        if not leaders:
            ...
            for i, row in enumerate(leaders):
                lines.append(f'{medals[i]} {mask_name(row["first_name"])} — {row["total_uc"]} UC')
            text = '\n'.join(lines)
        edit_menu(user_id, text, back_to_menu_markup())
        return
